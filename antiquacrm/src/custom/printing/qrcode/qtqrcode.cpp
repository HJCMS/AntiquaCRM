...
QtQrCode::QtQrCode(const QByteArray &data, int version,
                   ...
                   QtQrCode::ErrorCorrectionLevel errorCorrectionLevel)
    : d(new QtQrCodeData) {
  constructorPrivate(data, version, encodeMode, proportion, caseSensitivity,
                     errorCorrectionLevel);
}

void QtQrCode::constructorPrivate(
    const QByteArray &data, int version, QtQrCode::EncodeMode encodeMode,
...
void QtQrCode::encodeData() {
  ...
  if (d->proportion == MicroProportion) {
    qrCode = QRcode_encodeDataMQR(
        d->data.size(), (const unsigned char *)d->data.constData(), d->version,
        (QRecLevel)d->errorCorrectionLevel);
  } else {
    qrCode = QRcode_encodeData(d->data.size(),
                               (const unsigned char *)d->data.constData(),
...
void QtQrCode::encodeString() {
  ...
  if (d->proportion == MicroProportion) {
    qrCode = QRcode_encodeStringMQR(
        QString(d->data).toStdString().c_str(), d->version,
        (QRecLevel)d->errorCorrectionLevel, QR_MODE_8, d->caseSensitivity);
  } else {
...
int QtQrCode::width() const { return d->width; }

int QtQrCode::version() const { return d->version; }
...
QByteArray QtQrCode::data() const { return d->data; }
...
QtQrCode::EncodeMode QtQrCode::encodeMode() const { return d->encodeMode; }
...
QtQrCode::Proportion QtQrCode::proportion() const { return d->proportion; }
...
void QtQrCode::setErrorCorrectionLevel(
...
bool QtQrCode::operator!=(const QtQrCode &other) { return !(*this == other); }
