...
PurchaseOverview::PurchaseOverview(const QString &id, QWidget *parent)
  ...
  // Remote Update Status Payment
  connect(m_paymentInfo, SIGNAL(sendUpdateOrderStatus(Antiqua::PaymentStatus)),
          this, SIGNAL(sendUpdateOrderStatus(Antiqua::PaymentStatus)));
}
...
void PurchaseOverview::setOrderUpdateActions(
    const QMap<Antiqua::PaymentStatus, QString> &map) {
  m_paymentInfo->setOrderUpdateActions(map);
}
