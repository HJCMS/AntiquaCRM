...
// BEGIN ACurl
ACurl::ACurl(QObject *parent, bool debug) : QThread{parent} {
  verbose = debug;
}

void ACurl::sendGet(const QUrl &url) {
  p_type = ACurl::GET;
  p_url = url;
  run();
}

void ACurl::sendPost(const QUrl &url, const QByteArray &body) {
  p_type = ACurl::POST;
  p_url = url;
  p_body = body;
  run();
...
void ACurl::sendPostForm(const QUrl &url, const QString &param,
                            const QByteArray &body) {
  p_type = ACurl::FORM;
  p_url = url;
  p_formdata = param;
...
const QJsonDocument ACurl::getDocument() { return p_json; }
...
void ACurl::run() {
  ...
  if (status) {
    ...
    emit sendErrors(req->getResponseError());
#ifdef ANTIQUA_DEVELOPEMENT
  ...
  p_mutex.unlock();
  emit sendResponsed(p_json);
  emit sendRequestFinished(status);
}
