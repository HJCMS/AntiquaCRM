...
Assistant::Assistant(QWidget* parent) : QDialog(parent) {
  ...
  // Signals
  connect(m_stackedWidget, SIGNAL(currentChanged(int)), this, SLOT(pageEntered(int)));
...
bool Assistant::connectionTest() {
  ...
  QSqlDatabase psql = QSqlDatabase::addDatabase("QPSQL", sql_conf.connectionName());
...
void Assistant::migrationData(const QString& section) {
  ...
  QSettings cfg(QSettings::NativeFormat, QSettings::UserScope, domain, "antiquacrm", this);
