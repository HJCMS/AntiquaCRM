...
DeliverServiceEdit::DeliverServiceEdit(QWidget *parent) : QWidget{parent} {
  ...
  d_class = new AntiquaCRM::TextLine(this);
  ...
  d_definition = new AntiquaCRM::TextLine(this);
  ...
  d_description = new AntiquaCRM::TextLine(this);
  ...
  QString info("<b>" + tr("Hints") + "</b><ul>");
  QString n1 = tr("Only the current edited entry can be saved!");
...
void DeliverServiceEdit::clearFields() {
  foreach (AntiquaCRM::AInputWidget *w, findChildren<AntiquaCRM::AInputWidget *>()) {
    w->reset();
  }
...
const DeliverItem DeliverServiceEdit::getSaveData() {
  ...
  QString name = d_class->getValue().toString();
  ...
  if (d_cid->getValue().toInt() > 0) {
    info.setPrimaryKey(d_cid->getValue().toInt());
  }
  info.setDefinition(d_definition->getValue().toString());
  info.setInternational(d_international->getValue().toBool());
  info.setDefault(d_default->getValue().toBool());
  info.setDescription(d_description->getValue().toString());
  info.setPrice(d_price->getValue().toDouble());
...
void DeliverServiceEdit::addDeliverServiceEntry(
  ...
  while (it.hasNext()) {
    ...
    AntiquaCRM::AInputWidget *i = findChild<AntiquaCRM::AInputWidget *>(it.key());
    if (i != nullptr) {
      i->setValue(it.value());
    }
  }
