...
void AdmonitionNote::setBodyLayout() {
  ...
  m_body->setCellItem(hcl, tr("Admonition"), Qt::AlignLeft);
...
bool AdmonitionNote::setContentData(QJsonObject &data) {
  ...
  m_body->insertText(companyData("COMPANY_PAYMENT_ADMONITION_OPEN"));
...
void PrintAdmonition::createPDF() {
  ...
  if (_dir.exists()) {
    ...
    renderPage(printer);
    if (_file.isReadable())
      sendStatusMessage(tr("Admonition - PDF created!"));
  }
}

void PrintAdmonition::openPrintDialog() {
