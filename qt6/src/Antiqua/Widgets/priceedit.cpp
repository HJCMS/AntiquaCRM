...
PriceEdit::PriceEdit(double min, double max, QWidget* parent) : AntiquaCRM::AInputWidget{parent} {
...
void PriceEdit::setMinimum(double min) {
  m_edit->setMinimum(min);
}

void PriceEdit::setMaximum(double max) {
  m_edit->setMaximum(max);
}
...
void PriceEdit::setInputToolTip(const QString& str) {
  m_edit->setToolTip(str);
}
...
const QVariant PriceEdit::getValue() {
  return m_edit->value();
}
...
const QString PriceEdit::popUpHints() {
  ...
  info << tr(
      "The minimum value specified in the configuration has been fallen "
