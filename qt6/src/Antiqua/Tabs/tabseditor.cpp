...
AntiquaCRM::AInputWidget* TabsEditor::getInputEdit(const QString& name) {
  return findChild<AntiquaCRM::AInputWidget*>(name, Qt::FindChildrenRecursively);
}

QList<AntiquaCRM::AInputWidget*> TabsEditor::getInputEditList(const QRegularExpression& pattern) {
  return findChildren<AntiquaCRM::AInputWidget*>(pattern, Qt::FindChildrenRecursively);
}
...
#ifdef ANTIQUA_DEVELOPMENT
      qDebug() << Q_FUNC_INFO << list.at(i)->objectName();
#endif
...
bool TabsEditor::isModifiedCompare(const QString& name, const QVariant& origin) {
...
void TabsEditor::unsavedChangesPopup() {
  AntiquaCRM::APopUpMessage* d = new AntiquaCRM::APopUpMessage(this);
  d->setUnsavedMessage(tr("Do not leave this page until you have saved your changes!"));
  d->exec();
  d->deleteLater();
}
