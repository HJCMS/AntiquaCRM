...
OrderAddArticle::OrderAddArticle(QWidget *parent) : QDialog{parent} {
...
OrderAddArticle::addItem(const QString &key, const QVariant &value) const {
  return AntiquaCRM::AProviderOrder::createItem(key, value);
}

bool OrderAddArticle::queryArticle(qint64 aid) {
  // Reset
  articleInfo.clear();
  articleId = -1;
...
void OrderAddArticle::setSearchArticle() {
...
void OrderAddArticle::clearSearchInput() { m_searchEdit->setValue(0); }

void OrderAddArticle::setInfoMessage(const QString &message) {
  m_statusBar->showMessage(message, (60 * 1000));
}

qint64 OrderAddArticle::getArticle() { return articleId; }

int OrderAddArticle::exec() {
  clearSearchInput();
